...
func main() {
	...
	// Load model
	model, err := ash.LoadModel("teapot.gltf")
	if err != nil {
	...
	projMatrix.Perspective(ash.DegreesToRadians(45.0), float32(windowWidth)/float32(windowHeight), 0.1, 100.0)
	viewMatrix.LookAt(&ash.Vec3{0.0, 3.0, 8.0}, &ash.Vec3{0.0, 0.0, 0.0}, &ash.Vec3{0.0, 1.0, 0.0})
	projMatrix[1][1] *= -1
